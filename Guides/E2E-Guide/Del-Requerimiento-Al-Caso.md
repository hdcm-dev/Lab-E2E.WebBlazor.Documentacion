---
doc_id: E2E-04
doc_type: metodo-propuesto
title: Del requerimiento al caso — cómo se deducen superficies, promesas y estados
status: vigente
origin: agente
confidence: media
owner: Lab-E2E.WebBlazor.Documentacion
last_review: 2026-09-09
audience: [desarrollo, qa, po, analisis]
traces: [E2E-00, E2E-02, E2E-03]
---
...
Conviene decirlo antes de que alguien lo use para decidir algo. **Este documento propone un método;
no describe una práctica ya asentada en los laboratorios.** Los tres casos de diseño del conjunto
parten de pantallas ya construidas, así que ninguno recorre el camino de ida que se escribe acá.

Lo que sí tiene respaldo, y lo que no, está en [§10](#10-qué-de-esto-tiene-evidencia-y-qué-no). La
regla de lectura mientras tanto: **donde dice [C] es criterio discutible, no hallazgo.**
...
| **[F: id]** | *Fundamentado*: respaldado por una fuente externa, citada en el [Marco](Marco-La-Superficie-Verificable.md) |
| **[C]** | *Criterio*: decisión de este laboratorio, defendible y discutible |

---

## 1. Por qué este eslabón se volvió necesario
...
## 2. Quién interviene, y qué se pregunta cada uno
...
ahí es donde alguien pregunta «¿esto son tres pruebas o una?» y no hay con qué contestarlo (§5).

## 3. El punto cero: con qué llega realmente quien pide
...
| «Quiero poner líneas de colectivo» | Una **decisión ya tomada** | El motivo, y qué la haría cambiar | **Subir con cuidado** → §3.1 |
...
| «No sé cómo funciona mi ciudad» | **Exploración legítima** | La hipótesis | **Acompañar** → §3.2 |
...
### 3.1. El caso incómodo: cuando ya decidió
...
### 3.2. Cuando no se sabe: la exploración
...
### 3.3. Lo que hay que conseguir: el criterio de relevancia
...
### 3.4. Cuando el pedido ya trae las necesidades de datos
...
interrogar la intención hace perder tiempo sin agregar nada. Se salta a §4 y listo.
...
**En resumen**, §3 aplica cuando el pedido llega vago. Cuando ya trae las necesidades de datos, se
entra directo en §4 y se vuelve acá solo por lo que quedó indeterminado.

## 4. Las cinco preguntas de ida
...
## 5. Qué recurso representa el acto

Es la decisión que la era 3 dejó sin regla (§1), y se puede dar una: **el recurso no se elige, se
...
### 5.1. El segundo eje: qué pasa con la mirada
...
### 5.2. Lo que **no** distingue: conservar lo cargado
...
## 6. Qué promesas y qué estados trae cada recurso
...
### 6.1. Las promesas son de dos familias
...
(§5.2). Es la forma más rápida de comprobar que la tabla dice lo que tiene que decir.

### 6.3. Cómo se distingue una promesa de un estado
...
### 6.4. Un rol distinto cambia los actos, y se nota
...
| Honesto (§3.1) | *«¿Y si los datos dicen que llegan bien?»* → «lo quiero saber igual» → **es una duda genuina, no un pedido de respaldo** |
...
Y §5: ocho campos derivados, nada se entrega en el medio, hay orden →
...
hipótesis (§3.2): *¿qué esperás encontrar, y qué te sorprendería?* De ahí salen tres o cuatro
...
## 9. Qué **no** se decide en este tramo
...
requerimiento ni un dato del dominio.** Son la respuesta de §5 a una propiedad del acto. Si mañana
...
# 10. Qué de esto tiene evidencia y qué no

La regla del conjunto es que toda afirmación esté respaldada por evidencia verificable. Este
documento la cumple **declarando dónde no la hay**, que es la única forma honesta de cumplirla en un
método que todavía no se ejercitó.

## 10.1. Lo que tiene respaldo
...
Las filas de [§5](#5-qué-recurso-representa-el-acto) y [§6](#6-qué-promesas-y-qué-estados-trae-cada-recurso) no valen todas lo mismo. **[C]**
...
descripción de algo observado. Concretamente, no están comprobadas:
...
## 10.4. Lo que es encuadre y no hallazgo

Las **tres eras** de [§1](#1-por-qué-este-eslabón-se-volvió-necesario) son una forma de contar por
qué la elección de recurso hoy hace falta y antes no. **No es una afirmación histórica documentada**:
no se citan fuentes de historia del diseño de interfaces, y no se investigaron. Es un encuadre útil,
y como tal está marcado **[C]**.

Lo mismo vale para las **cinco alturas** de [§3](#3-el-punto-cero-con-qué-llega-realmente-quien-pide)
y para el **propósito mínimo** de [§3.4](#34-cuando-el-pedido-ya-trae-las-necesidades-de-datos):
salen de ordenar situaciones reconocibles, no de un relevamiento de casos registrados.

---

# 11. Los criterios de este documento, en una lista
...
13. **El recurso se decide con dos ejes.** El estructural dice cuántas superficies hay y descarta lo
    incompatible; el atencional elige entre lo que queda.
14. **Conservar lo cargado no distingue ningún recurso: es transversal.** Y por eso es la promesa que
    más se da por sentada — lo trivial es justamente lo que nadie escribe.
15. **Las promesas son de dos familias**: la del acto sobrevive al cambio de recurso, las del recurso
    se reemplazan con él. Si al cambiar el recurso se rompe el caso central, la promesa afirmaba el
    cómo.
